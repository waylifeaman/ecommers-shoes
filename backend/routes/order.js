    ...
    try {
        ...
        for (const item of items) {
            const [result] = await conn.query(
                ...
                [item.qty, item.product_id, item.size, item.qty]
            );

            if (result.affectedRows === 0) {
                await conn.rollback();
                return res.status(400).json({ error: "Stok tidak mencukupi untuk salah satu item" });
            }
        }

        // 6. Hapus item dari carts
        await conn.query(`DELETE FROM carts WHERE id IN (${placeholders})`, cart_ids);
        ...
        conn.release(); // wajib, supaya koneksi kembali ke pool
    }
});
